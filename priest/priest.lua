...
local function combat()
  ...
  -- Heal
  if dark_addon.settings.fetch('cpl_heal.check', false)
  ...
  -- Lesser Heal
  if dark_addon.settings.fetch('cpl_lesserheal.check', false)
  ...
  -- Renew
  if dark_addon.settings.fetch('cpl_renew.check', false)
  ...
  if toggle('dps', false) then
    -- Shadow Word Pain
    if dark_addon.settings.fetch('cpl_swp.check', true)
    and target.health.actual > dark_addon.settings.fetch('cpl_swp.spin', 50)
    and castable(SB.ShadowWordPain)
    and not target.debuff(SB.ShadowWordPain).exists then
      return cast(SB.ShadowWordPain, target)
    end
    -- Mind Blast
    if dark_addon.settings.fetch('cpl_mindblast', true)
    and castable(SB.MindBlast)
    and not player.moving then
      return cast(SB.MindBlast, target)
    end
    -- Smite
    if dark_addon.settings.fetch('cpl_smite.check', true)
    and player.power.mana.percent > dark_addon.settings.fetch('cpl_smite.spin', 50)
    and castable(SB.Smite)
    and not player.moving then
      return cast(SB.Smite, target)
    end
    -- Shoot (Wand)
    if dark_addon.settings.fetch('cpl_shoot.check', true)
